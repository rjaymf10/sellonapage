          ...
          <div class="different-address">
            ...
            <div id="ship-box-info" class="row">
              ...
              <div class="col-md-12">
                <div class="country-select">
                  <label>State / County <span class="required">*</span></label>										
                  <select id="shipping_state"></select>
                </div>
              </div>
              <div class="col-md-12">
                <div class="checkout-form-list">
...
<script>
  $(document).ready(function() {
      ...
      $.getJSON('https://raw.githubusercontent.com/dr5hn/countries-states-cities-database/master/countries.json')
        .done(function (data) {
          $.each(data, function(i, item) {
            ...
            $('#shipping_country').append($('<option>', { 
              value: item.id,
              text : item.name 
            }));
          });
    ...
    function states(country_id, type) {
    ...
    $("#billing_country").change(function(event){
      states($("#billing_country").val(), 'billing');
    });

    $("#shipping_country").change(function(event){
      states($("#shipping_country").val(), 'shipping');
    });

    // $("#billing_state").change(function(event){
    //   cities($("#billing_state").val(), 'billing');
    // });
